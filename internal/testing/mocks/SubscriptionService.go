...
// NotifyAboutNewVideo provides a mock function with given fields: ctx, channelID
func (_m *SubscriptionService) NotifyAboutNewVideo(ctx context.Context, channelID int) error {
	ret := _m.Called(ctx, channelID)

	if len(ret) == 0 {
		panic("no return value specified for NotifyAboutNewVideo")
	}
