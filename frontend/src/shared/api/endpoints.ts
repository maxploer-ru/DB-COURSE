...
export type VideoSort = 'newest' | 'views' | 'rating'

export const videoApi = {
  list: async (params?: { limit?: number; offset?: number; sort?: VideoSort }) => {
    const { data } = await apiClient.get<VideoApiResponse[]>('/videos', { params })
    return data.map(mapVideo)
  },
  listMine: async (params?: { limit?: number; offset?: number; sort?: VideoSort }) => {
    const { data } = await apiClient.get<VideoApiResponse[]>('/videos/me', { params })
    return data.map(mapVideo)
  },
  listByChannel: async (channelId: number, params?: { limit?: number; offset?: number; sort?: VideoSort }) => {
