...
Для демонстрации GraphQL именно в виде диаграммы типов откройте:

```text
http://localhost:4000/graph
```

Это визуальная схема основных узлов и связей: `User`, `Channel`, `Video`,
`Playlist`, `Comment`, `Community` и пагинация. Полный контракт остаётся
доступен через introspection и `/schema.graphql`.
